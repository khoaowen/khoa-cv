...
generates the PDFs, and deploys `dist/` to Cloudflare Pages.

One-time setup:

1. Create a Cloudflare Pages project named `khoa-cv` (Direct Upload).
2. Create a scoped API token (**Account → Cloudflare Pages → Edit**).
3. Add repo secrets:
   ```bash
